...
describe("agent transport routing", () => {
  ...
  it("keeps localhost FastAPI compatibility while making published domains same-origin", () => {
    expect(resolvePreviewApiBase("http:", "localhost")).toBe("http://127.0.0.1:8000");
    expect(resolvePreviewApiBase("https:", "alertify-das-vnrc64kb.manus.space")).toBe("https://alertify-das-vnrc64kb.manus.space");
  });
});
